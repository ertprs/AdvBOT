...
const tiktok = (url) => new Promise((resolve, reject) => {
    ...
    getVideoMeta(url, { noWaterMark: true, hdVideo: true,     headers: {
        'User-Agent': "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_3) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.122 Safari/537.36",
        Referer: 'https://www.tiktok.com/',
        Cookie: `tt_webid_v2=68dssds`,
    }, })
        .then(async (result) => {
            console.log('Get Video From', '@' + result.collector.authorMeta.name, 'ID:', result.id)
...
const facebook = (url) => new Promise((resolve, reject) => {
    console.log('Get metadata from =>', url)
    const apikey = 'owolc0ef1ROgY6rNXXYjfJUBf3dDqKWv5KdojgrkP4ynIYAB2Z'
