...
const covid = require('./lib/covid-19')
        ...
        client.getGroupMembersId(id)
            ...
            client.onGlobalParicipantsChanged(async (event) => {
                ...
                if (event.action === 'remove' && event.who !== host) {
                    await client.sendFileFromUrl(event.chat, profile, 'profile.jpg', '')
                    await client.sendTextWithMentions(event.chat, `Adeus @${event.who.replace('@c.us', '')} ✨`)
                }
            })

    client.onIncomingCall(( async (call) => {
        await client.sendText(call.peerJid, 'Não posso receber chamadas. Favor não Insista.')
    }))
}
